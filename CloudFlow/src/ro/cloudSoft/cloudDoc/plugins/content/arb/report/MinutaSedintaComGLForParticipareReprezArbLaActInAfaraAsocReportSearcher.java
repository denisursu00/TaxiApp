...
public class MinutaSedintaComGLForParticipareReprezArbLaActInAfaraAsocReportSearcher {
	...
	public MinutaSedintaComGLForParticipareReprezArbLaActInAfaraAsocReportSearcher(Repository repository, Credentials credentials, DocumentIdentifier documentIdentifierPrezentaComGlfilter,
	...
	private Query createQuery(Session session) throws RepositoryException {
		...
		queryBuilder.append(" AND (UPPER(document.[" + locatieJrPropertyName + "]) not like '%" + CalendarConstants.CALENDAR_EVENT_LOCATION_VALUE_ARB.toUpperCase() + "%' "
				+ "				OR document.[" + locatieJrPropertyName + "] is null ) ");	
