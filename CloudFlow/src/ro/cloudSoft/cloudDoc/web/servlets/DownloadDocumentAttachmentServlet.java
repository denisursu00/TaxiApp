...
public class DownloadDocumentAttachmentServlet extends HttpServlet {

	private static final long serialVersionUID = 1L;

	private static final LogHelper LOGGER = LogHelper.getInstance(DownloadDocumentAttachmentServlet.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		final String attachmentName = request.getParameter(PARAMETER_ATTACHMENT_NAME);
		...
		if (StringUtils.isNotBlank(documentLocationRealName) && StringUtils.isNotBlank(documentId)) {
			...
			// FileSystemAttachmentManager fileSystemAttachmentManager = new FileSystemAttachmentManager();
			FileSystemAttachmentManager fileSystemAttachmentManager = SpringUtils.getBean("fileSystemAttachmentManager");
			byte[] content = fileSystemAttachmentManager.getAttachmentAsByteArray(attachmentName, username);
			if (content == null) {
			...
			attachment = new Attachment(attachmentName, content);
		}
