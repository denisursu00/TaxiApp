...
public class CerereConcediuNepersonalInsertHolidayEventInCalendar extends AutomaticTask {
	...
	public void execute(WorkflowInstance workflowInstance) throws AutomaticTaskExecutionException {
		...
		String tipConcediu = DocumentUtils.getMetadataListValueAsLabel(document, cerereConcediuDocumentType, getDocumentCerereConcediuConstants().getTipConcediuNepersonalMetadataName());
		...
		HolidayCalendarEvent calendarEvent = prepareCalendarEvent(startDate, endDate, beneficiar, tipConcediu, numeBeneficiarConcediu, calendar);
		calendarEvent.setDocumentId(document.getId());
		calendarEvent.setDocumentLocationRealName(document.getDocumentLocationRealName());
		
		getCalendarEventDao().saveEvent(calendarEvent);
	}
