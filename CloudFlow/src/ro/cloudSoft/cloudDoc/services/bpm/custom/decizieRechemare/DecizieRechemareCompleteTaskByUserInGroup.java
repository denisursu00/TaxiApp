...
public class DecizieRechemareCompleteTaskByUserInGroup extends AutomaticTask {
	...
	public void execute(WorkflowInstance workflowInstance) throws AutomaticTaskExecutionException {
		...
		for (Task task : tasks) {
			...
			if (isUserInGroup(workflowInstance)) {
				transitionName = getDocumentDecizieRechemareConstants().getInitiatorPresedinteBOMTransitionName(); 
			} else {
				transitionName = getDocumentDecizieRechemareConstants().getInitiatorDirectorExecutivTransitionName();
			}
	...
	private boolean isUserInGroup(WorkflowInstance workflowInstance) throws AutomaticTaskExecutionException  {
		...
		DocumentType decizieRechemareDocumentType = getDocumentTypeDao().getDocumentTypeByName(cerereRechemareDocumentTypeName);
		Document document;
		...
		Long initiatorDecizieRechemareId = DocumentUtils.getMetadataUserValue(document, decizieRechemareDocumentType, getDocumentDecizieRechemareConstants().getInitiatorDecizieRechemareMetadataName());
		Long groupId = DocumentUtils.getMetadataGroupValue(document, decizieRechemareDocumentType, getDocumentDecizieRechemareConstants().getGrupPresedinteBomMetadataName());
		return getGroupService().isUserInGroupWithId(initiatorDecizieRechemareId, groupId);
	}
