...
public class CerereConcediuCompleteTaskByEstablishingUserFunctionForNonPersonalApplication extends AutomaticTask {
	...
	public void execute(WorkflowInstance workflowInstance) throws AutomaticTaskExecutionException, AppException {
		...
		for (Task task : tasks) {
			...
			String transitionName = getDocumentCerereConcediuConstants().getBeneficiarCerereNepersonalaDiferitDePEDirectorExecutivTransitionName();
			if (getUserGroupService().isUserInGroupWithId(idBeneficiarConcediu, idGrupDirectori)) {
				transitionName = getDocumentCerereConcediuConstants().getBeneficiarCerereNepersonalaDirectorExecutivTransitionName();
			} else if (getUserGroupService().isUserInGroupWithId(idBeneficiarConcediu, idGrupPe)) {
				transitionName = getDocumentCerereConcediuConstants().getBeneficiarCerereNepersonalaPETransitionName();
			}	
