package ro.cloudSoft.cloudDoc.services.bpm.custom.ordineDeZiCd;
...
public class OrdineDeZiCdInsertEventInCalendar extends AutomaticTask {

	private static final String PREFIX_EVENT_DESCRIPTION = "Sedinta CD cu privire la";
	
	@Override
	public void execute(WorkflowInstance workflowInstance) throws AutomaticTaskExecutionException {
		
		String ordineDeZiDocumentTypeName = getDocumentOrdineDeZiCdConstants().getDocumentTypeName();
				
		DocumentType ordineDeZiDocumentType = getDocumentTypeDao().getDocumentTypeByName(ordineDeZiDocumentTypeName);
		Document document;
		...
		Date startDate = DocumentUtils.getMetadataDateTimeValue(document, ordineDeZiDocumentType, getDocumentOrdineDeZiCdConstants().getDataInceputMetadataName());
		Date endDate = DocumentUtils.getMetadataDateTimeValue(document, ordineDeZiDocumentType, getDocumentOrdineDeZiCdConstants().getDataSfarsitMetadataName());
		String location = DocumentUtils.getMetadataValueAsString(document, ordineDeZiDocumentType, getDocumentOrdineDeZiCdConstants().getLocatieMetadataName());
		Long calendarId = DocumentUtils.getMetadataCalendarValue(document, ordineDeZiDocumentType, getDocumentOrdineDeZiCdConstants().getCalendarMetadataName());
		String denumireEveniment = DocumentUtils.getMetadataValueAsString(document, ordineDeZiDocumentType, getDocumentOrdineDeZiCdConstants().getDenumireEvenimentMetadataName());
		
		if (startDate == null || endDate == null || location == null || calendarId == null || denumireEveniment == null) {
		...
		MeetingCalendarEvent meetingCalendarEventModel = prepareMeetingCalendarEvent(startDate, endDate, location, calendar, denumireEveniment);
	...
	private MeetingCalendarEvent prepareMeetingCalendarEvent(Date startDate, Date endDate, String location, Calendar calendar, String denumireEveniment) {
		...
		String denumireEvenimentWithPrefix = PREFIX_EVENT_DESCRIPTION + " " + denumireEveniment.trim();
	...
	public DocumentOrdineDeZiCdConstants getDocumentOrdineDeZiCdConstants() {
		return SpringUtils.getBean("documentOrdineDeZiCdConstants");
	}
}
