...
public class DspCompleteTaskByConditionResponsibleUserInGroup extends AutomaticTask {
	...
	public void execute(WorkflowInstance workflowInstance) throws AutomaticTaskExecutionException, AppException {
		...
		for (Task task : tasks) {
			...
			String transitionName = "Functie responsabil stabilita - verificare participanti superior";
			if (isUserInGroup(workflowInstance)) {
				transitionName = "Functie responsabil stabilita - verificare participanti director";
			}
	...
	private boolean isUserInGroup(WorkflowInstance workflowInstance) throws AppException {
		...
		for (MetadataInstance metadataInstance : documentMetadataInstanceList) {
			...
			if (metadataDefinition.getName().equals("responsabil")) {
				Long initiatorId = MetadataValueHelper.getUserId(metadataInstance.getValue());
				responsablil = getUserService().getUserById(initiatorId);
			}
			if (metadataDefinition.getName().equals("grup_directori")) {
				groupId = MetadataValueHelper.getGroupId(metadataInstance.getValue());
			}
		}
		
		for (Group group : responsablil.getGroups()) {
