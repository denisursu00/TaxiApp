...
import { AppError, TranslateUtils, DateConstants, ArrayUtils, ObjectUtils, StringUtils, AttributeTypeEnum, NomenclatorAttributeSelectionFilterModel, NomenclatorService, NomenclatorModel, NomenclatorAttributeModel, FormUtils, NomenclatorUtils } from "@app/shared";
...
@Component({
	selector: "app-nomenclator-attribute-definition-selection-filters-field",
	template: `
		...
		<p-dialog [(visible)]="dialogVisible" 
			...
			<form [formGroup]="form">
				...
				<div *ngIf="attributeKeyForAutocompleteFilterValueFormControl.enabled" class="ui-grid ui-grid-responsive ui-grid-pad ui-fluid">
					<div class="ui-grid-row">
						<div class="ui-grid-col-12">
							Attribute for autocomplete filter value:
						</div>
					</div>
					<div class="ui-grid-row">
						<div class="ui-grid-col-12">
							<input type="text" formControlName="attributeKeyForAutocompleteFilterValue" pInputText>
						</div>
					</div>
					<div class="ui-grid-row">
						<div class="ui-grid-col-12">
							<div class="ui-message ui-messages-error ui-corner-all" *ngIf="attributeKeyForAutocompleteFilterValueFormControl.invalid && (attributeKeyForAutocompleteFilterValueFormControl.dirty || attributeKeyForAutocompleteFilterValueFormControl.touched)">
								<span class="fa fa-close"></span>					
								<span *ngIf="attributeKeyForAutocompleteFilterValueFormControl.errors['required']">{{'MESSAGES.VALIDATOR_MANDATORY_FIELD' | translate}}</span>
							</div>
						</div>
					</div>
				</div>
	...
	`,
	providers: [
		{ provide: NG_VALUE_ACCESSOR, useExisting: NomenclatorAttributeDefinitionSelectionFiltersFieldComponent, multi: true }
	]
})
export class NomenclatorAttributeDefinitionSelectionFiltersFieldComponent implements ControlValueAccessor, OnChanges, OnInit {

	private fieldValue: ValueOfNomenclatorAttributeSelectionFilterField;
	...
	public ngOnInit(): void {
		...
		this.columns.push({header: "Attribute code for autocomplete filter value", field: "attributeKeyForAutocompleteFilterValue"});	
		...
		this.form.addControl("attributeKeyForAutocompleteFilterValue", new FormControl({value:null, disabled: true}));
	...
	private prepareFilterViews(): void {
		...
		this.loadNomenclatorModel(() => {
			...
			this.fieldValue.selectionFilters.forEach((filter: NomenclatorAttributeSelectionFilterModel, index: number) => {				
				...
				filterView.attributeKeyForAutocompleteFilterValue = filter.attributeKeyForAutocompleteFilterValue;
	...
	public writeValue(value: ValueOfNomenclatorAttributeSelectionFilterField): void {
	...
	public onDialogOKAction(event: any): void {
		...
		let newFilter: NomenclatorAttributeSelectionFilterModel = new NomenclatorAttributeSelectionFilterModel();
		...
		newFilter.attributeKeyForAutocompleteFilterValue = ObjectUtils.isNotNullOrUndefined(this.attributeKeyForAutocompleteFilterValueFormControl.value) ? this.attributeKeyForAutocompleteFilterValueFormControl.value.trim() : null;
	...
	private doWhenCloseDialog(): void {
		...
		this.attributeKeyForAutocompleteFilterValueFormControl.disable();
	}
	...
	public onFilterAttributeSelectionChanged(event: any): void {
		...
		FormUtils.enableOrDisableFormControl(this.attributeKeyForAutocompleteFilterValueFormControl, enableFilterValueFields);
	...
	public get attributeKeyForAutocompleteFilterValueFormControl(): FormControl { 
		return this.getFormControlByName("attributeKeyForAutocompleteFilterValue"); 
	}
}

export class ValueOfNomenclatorAttributeSelectionFilterField {

	public selectionFilters: NomenclatorAttributeSelectionFilterModel[];
...
class SelectionFilterView {
	...
	public attributeKeyForAutocompleteFilterValue: string;
}