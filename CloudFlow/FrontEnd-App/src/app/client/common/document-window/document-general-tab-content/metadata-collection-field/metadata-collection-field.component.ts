...
@Component({
	selector: "app-metadata-collection-field",
	templateUrl: "./metadata-collection-field.component.html",
	providers: [
		{ provide: NG_VALUE_ACCESSOR, useExisting: MetadataCollectionFieldComponent, multi: true }
	]
})
export class MetadataCollectionFieldComponent implements ControlValueAccessor, OnInit {

	@Input()
	public inputData: MetadataCollectionFieldInputData;
	...
	private prepareColumns(): void {
		this.columns = [];
		this.inputData.metadataCollectionDefinition.metadataDefinitions.forEach((metadataDefinition: MetadataDefinitionModel) => {
			if (MetadataUtils.isMetadataVisible(metadataDefinition, this.inputData.documentWorkflowState)) {
				this.columns.push({
...
export interface MetadataCollectionFieldInputData {
